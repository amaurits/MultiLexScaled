...
def calibrate_features(valencefile, valencecols, neutralscaler='', stdev_adj=1,
    ...
    # Load data.
    # Note: we will usually have an id column at column 0, but if it contains duplicates that will cause an error
    # Instead, just use the temporary index generated by pandas, and be sure to delete it at the end
    df = pd.read_csv(valencefile) if header else \
            pd.read_csv(valencefile, names=colnames)
    ...
    df_towrite = calibrated_df if len(keepcols) == 0 else \
        pd.concat([df[keepcols], calibrated_df], axis=1, join='inner')
    outputfile = '.'.join(valencefile.split('.')[:-1]) + outsuffix + '.csv'
    df_towrite.to_csv(outputfile, index=False)
